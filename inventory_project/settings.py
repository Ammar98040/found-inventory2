...
# إعدادات CSRF - يمكن إضافة النطاقات عبر CSRF_TRUSTED_ORIGINS في .env
_origins = config('CSRF_TRUSTED_ORIGINS', default='').split(',')
CSRF_TRUSTED_ORIGINS = [o.strip() for o in _origins if o.strip()] or [
    'http://127.0.0.1:8000', 'http://127.0.0.1:8001',
    'http://localhost:8000', 'http://localhost:8001',
    'http://0.0.0.0:8000', 'http://0.0.0.0:8001',
...
# إعدادات CSRF (Lax أفضل توافق مع VPS والبروكسي العكسي)
CSRF_COOKIE_HTTPONLY = True
CSRF_COOKIE_SAMESITE = config('CSRF_COOKIE_SAMESITE', default='Lax')
CSRF_FAILURE_VIEW = 'inventory_app.views.csrf_failure'

# إعدادات Session (Lax يسمح بإرسال الكوكي مع طلبات fetch من نفس الموقع على VPS)
